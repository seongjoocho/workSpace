...
/*
 ...
 2. 사용자는 가진 구슬 개수를 걸고 홀/짝 중 하나를 선택.
 3. 결과값이 화면에 보여진다.
 */
...
class ViewController: UIViewController {
    ...
    @IBAction func gameStarPressed(_ sender: Any) {
        ...
        alert.addTextField { textField in
            textField.placeholder = "베팅 할 구슬의 개수를 입력해주세요."
        }
    ...
    func calculateBalls(winner: String, count: Int){
        ...
        if winner == "com" {
            ...
            self.comBallsCout = self.comBallsCout + count
        }else{
            self.userBallsCount = self.userBallsCount + count
            self.comBallsCout = self.comBallsCout - count
        }
        self.userBallCountLbl.text = "\(self.userBallsCount)"
        self.computerBallCountLbl.text = "\(self.comBallsCout)"
